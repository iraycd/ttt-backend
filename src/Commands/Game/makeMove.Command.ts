...
export default class MakeMoveCommand {
  ...
  canMakeMove(moves, presentGamer, initialGamer) {
    ...
    if (presentGamer == initialGamer) {
      ...
      // Other
      canMoveValidation =
        games[presentGamer].length < games[initialGamer].length;
    }
  ...
  async run() {
    ...
    const gamerInGameCheck = builder("gamers_in_game", {
      _from: gamerValue._id,
      _to: gameValue._id
    });
    const gamerInGameCheckQueryCursor = await this.arangoDI.db.query(
      gamerInGameCheck.query,
      gamerInGameCheck.values
    );
    NotFound.assert(
      gamerInGameCheckQueryCursor._result.length > 0,
      "Gamer does not exist in the game"
    );

    // Check if the gamer belongs to the game.
    const gamerCountInGameCheck = builder("gamers_in_game", {
      _to: gameValue._id
    });
    const gamerCountInGameCheckCursor = await this.arangoDI.db.query(
      gamerCountInGameCheck.query,
      gamerCountInGameCheck.values
    );
    NotFound.assert(
      gamerCountInGameCheckCursor._result.length <= gameValue.minPlayers,
      "Not enough gamers to start the game"
    );

    // Validate if the move was not already used
    const moveExistingCheckQuery = builder("move", {
      game: gameValue._key,
      row: this.value.row,
      col: this.value.col
    });
    const moveExistingCheckQueryCursor = await this.arangoDI.db.query(
      moveExistingCheckQuery.query,
      moveExistingCheckQuery.values
    );
    NotFound.assert(
      moveExistingCheckQueryCursor._result.length == 0,
      "Move already registered"
    );

    // Check if it's the correct player who has made the move.
    const gameMoveQuery = builder("move", { game: gameValue._key });
    const gameMoveQueryCursor = await this.arangoDI.db.query(
      gameMoveQuery.query,
      gameMoveQuery.values
    );
    ...
    if (initialGamer && moves.length > 0) {
      ...
      await this.gameModel.update(gameValue._key, {
        firstPlayer: gamerValue._key
      });
    }
    // Check the game
    await this.model.save(this.value);
    const newGameMoveQueryCursor = await this.arangoDI.db.query(
      gameMoveQuery.query,
      gameMoveQuery.values
    );
